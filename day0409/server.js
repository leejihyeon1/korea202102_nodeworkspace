...
const conStr={//바꿀 일이 없기때문에 const로 고정!
    user:"node",
    password:"node",
            ...
            con.execute(sql,function(error,result){
                if(error){
                    ...
                    con.execute(sql,[news_id],function(e,record){
                        if(e){
                            console.log("코멘트목록 가져오는 중 에러 발생",e);
                        }else{
                            response.render("news/detail",{
                                news:result.rows[0],//뉴스목록
    ...
    oracledb.getConnection(conStr,function(err,con){
        if(err){
            ...
            con.execute(sql,[news_id,msg,author],function(error,result){
                if(error){
                    console.log("insert 쿼리 실행 중 에러 발생",error);
                    // response.writeHead(500,{"Content-Type":"text/html;charset=utf-8"});
                    ...
                    // 클라이언트가 댓글 등록 요청을 비동기방식으로 요청했기 때문에, 클라이언트의 브라우저는
                    // 화면이 유지되어야 한다. 따라서 서버는 클라이언트가 보게될 디자인 코드를 보낼 이유가 없다..
                    // 왜? 보내는 순간 화면이 바뀌어 버리므로,,(이것은 클라이언트가 원하는게 아님)
                    // 그럼 뭐를 보내야 하나? 디자인 일부에 사용할 데이터만 보내면 된다!!!!!!

                    response.writeHead(200,{"Content-Type":"text/json;charset=utf-8"});//html--->json
                    //네트워크 상으로 주고받는 데이터는 문자열화 시켜서 주고 받는다!
                    var str="";
                    str+="{";
                    str+=" \"result\":\"1\"";
                    str+="}";
                ...
                con.close();//mysql-con.end(); oracle-con.close();
            })
        }
    })
})
app.get("/comments/list",function(request,response){//코멘트 목록 가져오기
        ...
        if(err){
            ...
            con.execute(sql,function(error,result,fields){
                if(error){
                    ...
                    /*
                    JSON.parse() : 문자열 --> json
                    JSON.stringify(json object) : json -->문자열
                    */
